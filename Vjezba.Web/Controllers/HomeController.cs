...
namespace Vjezba.Web.Controllers
{
	public class HomeController : Controller
	{
		...
		[Route("cesto-postavljana-pitanja/{selected:minlength(1):maxlength(2):int?}")]
		...
		///     URL ove akcije je /Home/SubmitQuery, uz POST zahtjev isključivo - ne može se napraviti GET zahtjev zbog [HttpPost]
		///     parametra
		/// </summary>
		...
		public IActionResult SubmitQuery(IFormCollection formData) {
			...
			StringValues ime = formData["ime"];
			StringValues prezime = formData["prezime"];
			StringValues email = formData["email"];
			StringValues poruka = formData["poruka"];
			StringValues tip = formData["tip"];
			bool newsletter = bool.Parse(formData["newsletter"].FirstOrDefault());

			string msg = "Poštovani {0} {1} ({2}) zaprimili smo vašu poruku te će vam se netko ubrzo javiti. Sadržaj vaše poruke je: [{3}] {4}." + " Također, {5} o daljnjim promjenama preko newslettera.";

			ViewBag.Message = string.Format(msg, ime, prezime, email, tip, poruka, newsletter ? "obavijestit cemo vas" : "necemo vas obavijestiti");
